...
function LawsPage() {
  ...
  const items = useMemo(() => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      result.push({ ...baseItem, key: `${baseItem.key}-${i}` });
    }
    return result;
  }, [baseItems]);
      ...
      if (user) {
        try {
          ...
          const progressPromises = baseItems.map(async (item) => {
            const progress = await getUserProgress(user.uid, item.key);
  ...
  const saveSimulation = async (lawId, simulationData) => {
    if (!user) return;
    try {
      const result = await saveSimulationResult(user.uid, {
        lawId,
        ...simulationData,
        timestamp: new Date().toISOString()
      });
      if (result) {
        setUserStats(prev => ({ ...prev, totalSimulations: (prev?.totalSimulations || 0) + 1 }));
      }
    } catch (error) {
  ...
  const saveProgress = async (lawId, progressData) => {
    if (!user) return;
    try {
      const result = await saveUserProgress(user.uid, lawId, progressData);
      if (result) {
        setUserProgress(prev => ({ ...prev, [lawId]: { ...prev[lawId], ...progressData } }));
      }
    } catch (error) {
  ...
  const openAestheticUsability = (version = 'basic') => { setGameType('aesthetic-usability'); setAestheticVersion(version); setAestheticTask(0); };
  const openDoherty = (delay = 100) => { setGameType('doherty'); setDohertyDelay(delay); setDohertyTask(0); };
  const openPragnanz = (version = 'complex') => { setGameType('pragnanz'); setPragnanzVersion(version); setPragnanzTask(0); };
  const openOccam = (version = 'long') => { setGameType('occam'); setOccamVersion(version); setOccamStep(0); };
  const openClickCost = (version = 'multi') => { setGameType('click-cost'); setClickCostVersion(version); setClickCostStep(0); };
  const openCommonRegion = () => { setGameType('common-region'); setCommonRegionGroups({}); setCommonRegionStart(Date.now()); };
  const openFittsEdge = () => { setGameType('fitts-edge'); setFittsEdgeStart(null); setFittsEdgeResult({ edge: null, center: null }); };
  const openProgressiveDisclosure = (version = 'all') => { setGameType('progressive-disclosure'); setProgressiveVersion(version); setProgressiveTask(0); };
  const openMicroInteractions = (version = 'basic') => { setGameType('micro-interactions'); setMicroVersion(version); setMicroAction(0); };
  const openNavigationPrimacy = (version = 'first') => { setGameType('navigation-primacy'); setNavPrimacyVersion(version); setNavPrimacyTarget('settings'); };
  const openVisibilityStatus = (version = 'with') => { setGameType('visibility-status'); setVisibilityVersion(version); setVisibilityProgress(0); };
  const openConsistency = (version = 'consistent') => { setGameType('consistency'); setConsistencyVersion(version); setConsistencyTask(0); };
            ...
            <div className="overlay-card" style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 16, padding: 24, width: 840 }}>
              ...
              <div className="prox-zones" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <div className="prox-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 40px)', gap: proxSpacing === 'tight' ? '8px' : '24px', justifyContent: 'center', margin: '10px 0 20px' }}>
                    {Array.from({ length: 16 }).map((_, i) => { const id = i + 1; return (<div key={id} draggable onDragStart={(e) => { e.dataTransfer.setData('text/plain', String(id)); if (!proximityStart) setProximityStart(Date.now()); }} style={{ width: 40, height: 40, borderRadius: 10, background: '#2d2d2d', display: 'grid', placeItems: 'center', color: '#888', cursor: 'grab' }}>{id}</div>); })}
                  </div>
                  <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={() => setProxSpacing('tight')} style={{ padding: '10px 16px', borderRadius: 10, background: proxSpacing === 'tight' ? '#8eb940' : '#333', color: proxSpacing === 'tight' ? '#000' : '#fff', border: '1px solid #555' }}>Jarak Dekat</button><button onClick={() => setProxSpacing('loose')} style={{ padding: '10px 16px', borderRadius: 10, background: proxSpacing === 'loose' ? '#8eb940' : '#333', color: proxSpacing === 'loose' ? '#000' : '#fff', border: '1px solid #555' }}>Jarak Jauh</button></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  {['A', 'B'].map(group => (<div key={group} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const id = e.dataTransfer.getData('text/plain'); setProximityGroups(prev => ({ ...prev, [id]: group })); }} style={{ minHeight: 220, border: '1px dashed #555', borderRadius: 12, padding: 12 }}><div style={{ marginBottom: 8, opacity: .8 }}>Group {group}</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{Object.entries(proximityGroups).filter(([_, g]) => g === group).map(([id]) => (<div key={id} style={{ width: 36, height: 36, borderRadius: 8, background: '#2d2d2d', display: 'grid', placeItems: 'center', color: '#888' }}>{id}</div>))}</div></div>))}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 16 }}>
                <button onClick={() => {
                  ...
                  resultDiv.innerHTML = `<div style="background:#1a1a1a; border:1px solid #333; border-radius:16px; padding:24px; max-width:500px; text-align:center"><h2 style="font-size:24px; margin-bottom:12px">Hasil Law of Proximity</h2><p>Waktu: <b>${timeMs}ms</b></p><p>Grouping benar: <b>${matches}/16</b></p><div style="background:#1a1a1a; border:1px solid #333; border-radius:12px; padding:16px; margin-top:20px"><h4 style="font-size:18px; margin-bottom:8px; color:#8eb940">💡 Lesson Learned:</h4><p style="font-size:14px; line-height:1.5; opacity:0.9"><strong>Law of Proximity terbukti:</strong> Elemen yang berdekatan dipersepsikan sebagai satu kelompok. Dalam desain UI, gunakan whitespace untuk memisahkan kelompok yang berbeda, letakkan elemen terkait berdekatan, dan gunakan visual grouping untuk meningkatkan scanability.</p></div><button onclick="this.parentElement.parentElement.remove()" style="margin-top:20px; padding:10px 20px; border-radius:10px; background:#8eb940; color:#000; border:none; font-weight:bold">Tutup</button></div>`;
        ...
        {gameType === 'peak-end' && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, color: '#fff' }}>
            <div className="overlay-card" style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 16, padding: 24, width: 600 }}>
              ...
              {peakEndStep === 0 && (<div><h3 style={{ fontSize: 20, marginBottom: 12 }}>Pertanyaan 1 (Mudah)</h3><p style={{ marginBottom: 16 }}>Berapa 2 + 2?</p><div className="rating-row" style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>{['3', '4', '5'].map((opt) => (<button key={opt} onClick={() => setPeakEndStep(1)} style={{ padding: '10px 20px', borderRadius: 10, background: '#333', color: '#fff', border: '1px solid #555' }}>{opt}</button>))}</div></div>)}
              {peakEndStep === 1 && (<div><h3 style={{ fontSize: 20, marginBottom: 12 }}>Pertanyaan 2 {peakEndFlow === 'B' ? '(Sulit)' : '(Mudah)'}</h3><p style={{ marginBottom: 16 }}>{peakEndFlow === 'B' ? 'Hitung integral dari x² dx dari 0 sampai 3' : 'Berapa 5 × 3?'}</p><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>{(peakEndFlow === 'B' ? ['9', '12', '18'] : ['12', '15', '18']).map((opt) => (<button key={opt} onClick={() => setPeakEndStep(2)} style={{ padding: '10px 20px', borderRadius: 10, background: '#333', color: '#fff', border: '1px solid #555' }}>{opt}</button>))}</div></div>)}
              {peakEndStep === 2 && (<div><h3 style={{ fontSize: 20, marginBottom: 12 }}>Pertanyaan 3 (Akhir Menyenangkan)</h3><p style={{ marginBottom: 16 }}>Apa warna favorit Anda?</p><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>{['Hijau', 'Biru', 'Ungu'].map((opt) => (<button key={opt} onClick={() => setPeakEndStep(3)} style={{ padding: '10px 20px', borderRadius: 10, background: '#333', color: '#fff', border: '1px solid #555' }}>{opt}</button>))}</div></div>)}
              {peakEndStep === 3 && (<div><h3 style={{ fontSize: 20, marginBottom: 12 }}>Rating Pengalaman</h3><p style={{ marginBottom: 16 }}>Bagaimana pengalaman mengisi quiz ini? (1-10)</p><div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: 16 }}>{Array.from({ length: 10 }, (_, i) => (<button key={i} onClick={() => setPeakEndRating(i + 1)} style={{ padding: '8px 12px', borderRadius: 8, background: peakEndRating === i + 1 ? '#8eb940' : '#333', color: peakEndRating === i + 1 ? '#000' : '#fff', border: '1px solid #555' }}>{i + 1}</button>))}</div>
                {peakEndRating && (<div><div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}><h4 style={{ fontSize: '18px', marginBottom: '8px', color: '#8eb940' }}>💡 Lesson Learned:</h4><p style={{ fontSize: '14px', lineHeight: '1.5', opacity: '0.9' }}><strong>Peak-End Rule terbukti:</strong> Pengalaman dinilai dari puncak emosional dan akhir, bukan rata-rata keseluruhan. Dalam desain UX, pastikan akhir pengalaman positif (confirmation, success message), minimalkan friction di tengah, dan ciptakan momen yang memorable.</p></div><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={() => { setPeakEndFlow(peakEndFlow === 'A' ? 'B' : 'A'); setPeakEndStep(0); setPeakEndRating(null); }} style={{ padding: '10px 16px', borderRadius: 10, background: '#333', color: '#fff', border: '1px solid #555' }}>Coba Flow {peakEndFlow === 'A' ? 'B' : 'A'}</button><button onClick={() => setGameType(null)} style={{ padding: '10px 16px', borderRadius: 10, background: '#8eb940', color: '#000', border: 'none', fontWeight: 'bold' }}>Tutup</button></div></div>)}
              </div>)}
            </div>
          </div>
        )}
        {gameType === 'serial-position' && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, color: '#fff' }}>
            <div className="overlay-card" style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 16, padding: 24, width: 600 }}>
              ...
              {serialShown ? (<div style={{ textAlign: 'center' }}><p style={{ marginBottom: 16 }}>Ingat kata-kata ini:</p><div className="serial-words" style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>{serialWords.map((word, i) => (<span key={i} style={{ padding: '8px 12px', background: '#333', borderRadius: 8, fontSize: 16 }}>{word}</span>))}</div></div>) : (<div><p style={{ marginBottom: 16 }}>Tuliskan kata-kata yang Anda ingat (pisahkan dengan koma):</p><textarea value={serialInput} onChange={e => setSerialInput(e.target.value)} style={{ width: '100%', height: 100, padding: 12, borderRadius: 10, border: '1px solid #555', background: '#111', color: '#fff', marginBottom: 16 }} placeholder="Apple, Banana, Cherry..." /><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={() => { const recalled = serialInput.split(',').map(s => s.trim().toLowerCase()); const positions = serialWords.map((word, i) => ({ position: i + 1, word: word.toLowerCase(), recalled: recalled.includes(word.toLowerCase()) })); setSerialResult(positions); }} style={{ padding: '10px 16px', borderRadius: 10, background: '#8eb940', color: '#000', border: 'none', fontWeight: 'bold' }}>Submit</button><button onClick={() => setGameType(null)} style={{ padding: '10px 16px', borderRadius: 10, background: '#333', color: '#fff', border: '1px solid #555' }}>Tutup</button></div>
                {serialResult && (<div style={{ marginTop: 16 }}><h3 style={{ fontSize: 18, marginBottom: 8 }}>Hasil Recall per Posisi:</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 16 }}>{serialResult.map((item, i) => (<div key={i} style={{ padding: 8, background: item.recalled ? '#8eb940' : '#333', borderRadius: 8, textAlign: 'center', fontSize: 12 }}><div>{item.position}</div><div style={{ opacity: 0.7 }}>{item.word}</div><div style={{ fontSize: 10 }}>{item.recalled ? '✓' : '✗'}</div></div>))}</div><div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: '12px', padding: '16px' }}><h4 style={{ fontSize: '18px', marginBottom: '8px', color: '#8eb940' }}>💡 Lesson Learned:</h4><p style={{ fontSize: '14px', lineHeight: '1.5', opacity: '0.9' }}><strong>Serial Position Effect terbukti:</strong> Item di awal (primacy) dan akhir (recency) lebih mudah diingat. Dalam desain UI, letakkan informasi penting di awal dan akhir list, gunakan breadcrumb untuk navigasi, dan highlight item terakhir yang diakses.</p></div></div>)}
              </div>)}
            </div>
          </div>
        )}
